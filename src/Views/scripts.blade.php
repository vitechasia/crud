<script>
    ...
    function dynamic_field(number) {
        ...
        html +=
            ...
            '" class="form-control" onchange="addenumfiled(this)"><option value="increments"> Increments </option><option value="integer"> Integer </option><option value="unsignedInteger"> unsignedInteger </option><option value="biginteger"> BigInteger </option><option value="unsignedBigInteger"> unsignedBigInteger </option><option value="string"> String </option><option value="date"> Date </option><option value="timestamps"> Timestamps </option><option value="text"> Text </option><option value="longtext"> LongText </option><option value="mediumtext"> MediumText </option><option value="boolean"> Boolean </option><option value="float"> Float </option><option value="double"> Double </option><option value="enum"> Enum </option></select><div id="enumfield' +
        ...
        if (number > 1) {
            html +=
                '<td class="p-1"><button class="btn btn-link text-danger remove" name="remove" type="button">x</button></td></tr>'
            $('#addnewitem').append(html);
        } else {
            html +=
                '<td class="p-1"><button class="btn btn-link text-success add" name="add" id="add" type="button">+</button></td></tr>'
    ...
    function relation_dynamic(num) {
        ...
        html +=
            '<td class="p-1" width="70"><center><button class="btn btn-link text-danger removerelation" id="removerelation" type="button">x</button></center></td></tr>'
